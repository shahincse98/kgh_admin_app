...
class _DispatchHistoryViewState extends State<DispatchHistoryView> {
  ...
  List<OrderModel> get filteredOrders {
    ...
    if (_selectedProductId.isNotEmpty) {
      list = list
          .where(
            (o) => o.items.any((item) => item.productId == _selectedProductId),
          )
          .toList();
    }
    if (_fromDate != null) {
  ...
  Widget build(BuildContext context) {
    ...
    final hasFilters =
        _fromDate != null || _toDate != null || _selectedProductId.isNotEmpty;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Obx(
          () => Text(
            'Delivered (${filteredOrders.length})',
                      ...
                      children: [
                        ...
                        Text(
                          _selectedProductId.isNotEmpty
                              ? '"$_selectedProductName" এর Delivered ${'তথ্য নেই'.tr}'
                              : 'কোনো Delivered অর্ডার নেই'.tr,
                        ),
                      ],
                    ),
                  );
                ...
                return RefreshIndicator(
                  ...
                  child: showGrouped
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (_, i) =>
                              _orderCard(filteredOrders[i], scheme),
                        ),
                );
              }),
            ),
  ...
  Widget _activeFilterChips(ColorScheme scheme) {
    ...
    if (_fromDate != null || _toDate != null) {
      final l =
          '${_fromDate != null ? _dayFmt.format(_fromDate!) : 'শুরু'} → ${_toDate != null ? _dayFmt.format(_toDate!) : 'শেষ'}';
      chips.add(
        _chip(
          l,
          () => setState(() {
    ...
    for (final o in filteredOrders) {
      for (final item in o.items) {
        if (item.productId == _selectedProductId) {
          ...
          final name = o.shopName.isNotEmpty ? o.shopName : 'Unknown'.tr;
          final memo = o.localMemo.isNotEmpty
              ? '#${o.localMemo}'
              : (o.memoNumber.isNotEmpty ? o.memoNumber : '');
        ...
        children: [
          Row(
            children: [
              ...
              Text(
                '$totalQtyটি • ${filteredOrders.length} ${'অর্ডার'.tr} • ${sorted.length} ${'কাস্টমার'.tr}',
                style: const TextStyle(
                  fontSize: 11,
                  color: Color(0xFF16A34A),
          ...
          const SizedBox(height: 6),
          ...sorted.map((e) {
            final memos = e.value.memos.isNotEmpty
                ? ' [${e.value.memos.join(', ')}]'
                : '';
            return Padding(
  ...
  Widget _searchBar(ColorScheme scheme) {
    return Padding(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: 'কাস্টমার নাম বা Order ID...'.tr,
                        ...
                        children: [
                          ...
                          if (order.deliveredAt != null)
                            _chip2(
                              ...
                              _dayFmt.format(order.deliveredAt!),
                              scheme,
                          ...
                          if (order.discountAmount > 0)
                            _chip2(
                              ...
                              '${'ডিসকাউন্ট'.tr}: ৳${_fmt.format(order.discountAmount.toInt())}',
                              scheme,
                              labelColor: const Color(0xFFD97706),
                            ),
                        ],
